...
def logged():
    ...
    def out():
        ...
        s = Surname.get()
        infoU2 = u,s, t, t2, dt
        uCom2 = "INSERT INTO Visitors (Name,Surname, Time, Time_out, Date) VALUES (%s , %s , %s, %s, %s)"

        mycursor.execute(uCom2, infoU2)
        mydb.commit()
        messagebox.showinfo()
    btnOut = tk.Button(outW, text="Out", command=out)
    btnOut.pack()

logbtn = tk.Button(root, text="SIGN-IN", bg='Magenta', command=logged)
logbtn.place(x=70, y=300, width=150)
